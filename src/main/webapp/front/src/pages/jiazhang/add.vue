...
<div :style='{"border":"2px solid #dbd9f4","padding":"20px","margin":"10px auto","borderRadius":"8px","background":"#fff","width":"1200px","position":"relative"}'>
    ...
    >
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"0"}' label="家长姓名" prop="jiazhangxingming">
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"0"}' label="班级名称" prop="banjimingcheng">
            <el-select  @change="banjimingchengChange" v-model="ruleForm.banjimingcheng" placeholder="请选择班级名称">
              <el-option
...
<script>
  export default {
    ...
    methods: {
      ...
      // 下二随
      banjimingchengChange () {
        this.$http.get('follow/banjixinxi/banjimingcheng?columnValue=' + this.ruleForm.banjimingcheng, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
            if(res.data.data.jiaoshigonghao){
              this.ruleForm.jiaoshigonghao = res.data.data.jiaoshigonghao
            }
          }
        });
      },
    ...
    // 多级联动参数
      // 多级联动参数
      info(id) {
        this.$http.get('jiazhang/detail/${id}', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
      ...
      onSubmit() {
              ...
              if(this.ruleForm.jiazhangzhanghao.length<3){
                this.$message.error('家长账号长度不能小于3');
                return
              }
              if(this.ruleForm.jiazhangzhanghao.length>10){
                  ...
                  params: params
                     ...
                     if(res.data.data.total>=crossoptnum) {
                         ...
                         // 跨表计算
                          ...
                          this.$http.post('jiazhang/add', this.ruleForm).then(res => {
                              if (res.data.code == 0) {
                                  this.$message({
                                      message: '操作成功',
                                      type: 'success',
