<template>
	<div class="addEdit-block" :style='{"padding":"30px 30px 30px 240px","backgroundAttachment":"fixed","background":"url(http://codegen.caihongy.cn/20220805/c242e6fd2047434bb1e519427e59e925.png)","backgroundSize":"cover","backgroundPosition":"center center","backgroundRepeat":"no-repeat","height":"calc(100vh - 120px)"}' style="width: 100%;">
		...
		>
			<template >
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="奖励原因" prop="jiangliyuanyin">
					<el-input v-model="ruleForm.jiangliyuanyin" placeholder="奖励原因" clearable  :readonly="ro.jiangliyuanyin"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' v-else class="input" label="奖励原因" prop="jiangliyuanyin">
					<el-input v-model="ruleForm.jiangliyuanyin" placeholder="奖励原因" readonly></el-input>
				</el-form-item>
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' v-if="type!='info'"  label="奖励详情" prop="jianglixiangqing">
					<editor 
						style="min-width: 200px; max-width: 600px;"
						v-model="ruleForm.jianglixiangqing" 
						class="editor" 
						action="file/upload">
					</editor>
				</el-form-item>
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' v-else-if="ruleForm.jianglixiangqing" label="奖励详情" prop="jianglixiangqing">
                    <span :style='{"fontSize":"14px","lineHeight":"40px","color":"#000","fontWeight":"500","display":"inline-block"}' v-html="ruleForm.jianglixiangqing"></span>
                </el-form-item>
...
export default {
	data() {
		...
		return {
			...
			ro:{
				jiangliyuanyin : false,
				jianglixiangqing : false,
	...
	methods: {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						if(o=='jiangliyuanyin'){
							this.ruleForm.jiangliyuanyin = obj[o];
							this.ro.jiangliyuanyin = true;
							continue;
						}
						if(o=='jianglixiangqing'){
							this.ruleForm.jianglixiangqing = obj[o];
							this.ro.jianglixiangqing = true;
							continue;
						}
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.jianglixiangqing = this.ruleForm.jianglixiangqing.replace(reg,'../../../ssm5umt6/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `jianglidengji/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.jianglidengjiCrossAddOrUpdateFlag = false;
						 this.parent.search();
