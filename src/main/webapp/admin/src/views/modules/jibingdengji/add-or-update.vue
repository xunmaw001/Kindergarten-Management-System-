<template>
	<div class="addEdit-block" :style='{"padding":"30px 30px 30px 240px","backgroundAttachment":"fixed","background":"url(http://codegen.caihongy.cn/20220805/c242e6fd2047434bb1e519427e59e925.png)","backgroundSize":"cover","backgroundPosition":"center center","backgroundRepeat":"no-repeat","height":"calc(100vh - 120px)"}' style="width: 100%;">
		...
		>
			<template >
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="疾病原因" prop="jibingyuanyin">
					<el-input v-model="ruleForm.jibingyuanyin" placeholder="疾病原因" clearable  :readonly="ro.jibingyuanyin"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' v-else class="input" label="疾病原因" prop="jibingyuanyin">
					<el-input v-model="ruleForm.jibingyuanyin" placeholder="疾病原因" readonly></el-input>
				</el-form-item>
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' v-if="type!='info'"  label="疾病详情" prop="jibingxiangqing">
					<editor 
						style="min-width: 200px; max-width: 600px;"
						v-model="ruleForm.jibingxiangqing" 
						class="editor" 
						action="file/upload">
					</editor>
				</el-form-item>
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' v-else-if="ruleForm.jibingxiangqing" label="疾病详情" prop="jibingxiangqing">
                    <span :style='{"fontSize":"14px","lineHeight":"40px","color":"#000","fontWeight":"500","display":"inline-block"}' v-html="ruleForm.jibingxiangqing"></span>
                </el-form-item>
...
export default {
	...
	methods: {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						if(o=='jibingyuanyin'){
							this.ruleForm.jibingyuanyin = obj[o];
							this.ro.jibingyuanyin = true;
							continue;
						}
						if(o=='jibingxiangqing'){
							this.ruleForm.jibingxiangqing = obj[o];
							this.ro.jibingxiangqing = true;
							continue;
						}
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.jibingxiangqing = this.ruleForm.jibingxiangqing.replace(reg,'../../../ssm5umt6/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `jibingdengji/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.jibingdengjiCrossAddOrUpdateFlag = false;
						 this.parent.search();
